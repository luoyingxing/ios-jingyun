...
//  Topevery.GPS 转换
//
//  Created by yeung on 16-8-17.
//  Copyright (c) 2016年 TYeung. All rights reserved.
//
...
- (CLLocation*)locationMarsFromEarthEx:(double)inLat withLng:(double)inLng
{
    double lat = 0.0;
    double lng = 0.0;
    transform_earth_from_mars(inLat, inLng, &lat, &lng);
    return [[CLLocation alloc] initWithCoordinate:CLLocationCoordinate2DMake(lat + inLat, lng + inLng)
                                         altitude:self.altitude
                               horizontalAccuracy:self.horizontalAccuracy
                                 verticalAccuracy:self.verticalAccuracy
                                           course:self.course
...
- (CLLocation*)locationEarthFromMarsEx:(double)inLat withLng:(double)inLng
{
    double lat = 0.0;
    double lng = 0.0;
    transform_earth_from_mars(inLat, inLng, &lat, &lng);
    return [[CLLocation alloc] initWithCoordinate:CLLocationCoordinate2DMake(inLat - lat, inLng - lng)
                                         altitude:self.altitude
                               horizontalAccuracy:self.horizontalAccuracy
                                 verticalAccuracy:self.verticalAccuracy
                                           course:self.course
